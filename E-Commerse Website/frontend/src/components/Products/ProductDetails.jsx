...
const ProductDetails = ({ productsId }) => {
    ...
    const productFetchId = productsId || id;
    useEffect(() => {
        if (productFetchId) {
            // dispatch(fetchProductDetails({ id: productFetchId }))
            dispatch(fetchProductDetails(productFetchId)); // ✅ Only ID, not an object
                    ...
                    <div className='flex flex-col md:flex-row'>
                        ...
                        <div className='hidden md:flex flex-col space-y-4 mr-6'>
                            {selectedProduct.images.map((image, index) => (
                                <img className={`w-20 h-20 object-cover rounded-lg cursor-pointer border ${mainImage == image.url ? "border-black" : "border-gray-300"}`} key={index} src={image.url} alt={image.altText || `Thumbnail ${index}`} onClick={() => setMainImage(image.url)} />
                            ))}
                        </div>
                        {/* Main Images */}
...
// error aala aahe aata sakdi karu yela solve